...
function App() {
    ...
    const saveProduct = async () => {
        ...
        if (error)
            ...
            alert('Saved to Oasis!');
    };
    if (!product && loading)
        return <div className="p-4">Scanning...</div>;
    return (<div className="w-[300px] p-4 bg-gray-900 text-white font-sans">
      ...
      {!session ? (<div className="text-center">
          ...
          {product ? (<div className="space-y-3">
              ...
              <button onClick={saveProduct} className="w-full bg-blue-600 hover:bg-blue-500 py-2 rounded font-bold mt-2" disabled={loading}>
                {loading ? 'Saving...' : 'Add to Oasis +'}
              </button>
            </div>) : (<p>No product detected on this page.</p>)}
        </>)}
    </div>);
}
export default App;
